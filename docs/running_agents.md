...
You can use the base [`RunResultBase.to_input_list()`][agents.result.RunResultBase.to_input_list] method to get the inputs for the next turn. Alternatively, configure an agent with session memory and provide a `session_id` in `RunConfig` so you only pass the new user message each time.
