...
class Runner:
    ...
    async def run(
        ...
        *,
        ...
        Args:
            ...
            session_id: The session ID for session memory. If the agent has session memory enabled
                and this is provided, the conversation history will be loaded and the new conversation
        ...
        session_memory = cls._get_session_memory(starting_agent)
        if session_memory and session_id:
            # Load existing conversation history and merge with new input
            existing_history = await session_memory.load_session(session_id)
            new_input_items = ItemHelpers.input_to_new_input_list(input)
            input = existing_history + new_input_items
                ...
                while True:
                    ...
                    if isinstance(turn_result.next_step, NextStepFinalOutput):
                        ...
                        if session_memory and session_id:
                            ...
                            await session_memory.save_session(
    ...
    @classmethod
    def _get_session_memory(cls, agent: Agent[Any]):
        """Get the session memory instance for the agent."""
        from .memory.interface import SessionMemory
        from .memory.sqlite_memory import SQLiteSessionMemory
